...
export const DashboardSummary: React.FC<DashboardSummaryProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Welcome Banner Card */}
      <div className="bg-gradient-to-r from-blue-900/90 via-[#1f283e] to-indigo-950/90 rounded-2xl p-6 text-white shadow-xl border border-blue-500/30 relative overflow-hidden">
        {/* Soft glow */}
        <div className="absolute -right-8 -bottom-10 w-48 h-48 bg-blue-500/15 rounded-full blur-3xl" />
        <div className="absolute right-20 -top-10 w-32 h-32 bg-indigo-500/15 rounded-full blur-2xl" />
        ...
        <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          ...
          <div className="flex items-center gap-3 shrink-0">
            ...
            <Link
              to="/recibos"
              className="bg-[#283046] hover:bg-[#303953] text-white font-semibold px-4 py-2.5 rounded-xl border border-[#36405c] transition text-sm flex items-center gap-2"
            >
              <Receipt className="w-4 h-4 text-blue-300" /> Ver Recibos
            </Link>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        {/* Recibos Pendientes */}
        <div className="bg-[#212738] p-5 rounded-2xl border border-[#2c344a] shadow-md hover:border-slate-600 transition">
          <div className="flex items-center justify-between mb-3">
            <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider">Recibos por Firmar</span>
            <div className={`p-2.5 rounded-xl border ${pendingPayslips.length > 0 ? 'bg-amber-500/15 text-amber-300 border-amber-500/30' : 'bg-[#181d2b] text-slate-400 border-[#2c344a]'}`}>
        ...
        <div className="bg-[#212738] p-5 rounded-2xl border border-[#2c344a] shadow-md hover:border-slate-600 transition">
          ...
          <div className="flex items-baseline gap-2">
            <span className="text-lg font-extrabold text-blue-300">{formatCurrency(payslips[0]?.netSalary || 0)}</span>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-[#212738] rounded-2xl border border-[#2c344a] p-6 shadow-md space-y-4">
          <div className="flex items-center justify-between border-b border-[#2c344a] pb-3">
          ...
          {pendingPayslips.length > 0 ? (
            ...
            <div className="py-8 text-center bg-[#181d2b] rounded-xl border border-[#2c344a] space-y-2">
        ...
        <div className="bg-[#212738] rounded-2xl border border-[#2c344a] p-6 shadow-md space-y-4">
          ...
          <div className="space-y-3">
            {recentLicenses.map((lic) => (
              <div 
                key={lic.id}
                className="p-3.5 rounded-xl bg-[#181d2b] border border-[#2c344a] flex items-center justify-between text-xs"
              >
                <div>
                  <div className="flex items-center gap-2">
