...
export const LicenseCard: React.FC<LicenseCardProps> = ({ license, onSelect }) => {
  return (
    <div 
      ...
      className="bg-[#212738] border border-[#2c344a] rounded-2xl p-5 shadow-md hover:shadow-lg transition-all cursor-pointer hover:border-blue-400/50 group relative overflow-hidden"
    >
      {/* Accent left line */}
      <div 
        className={`absolute left-0 top-0 bottom-0 w-1.5 ${
          ...
          license.status === 'en_revision' ? 'bg-blue-500' : 'bg-amber-500'
        }`}
      />

      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-3">
        <div className="flex items-center gap-2">
          <span className="text-xs font-mono font-semibold px-2 py-0.5 rounded bg-[#181d2b] text-slate-300 border border-[#2c344a]">
            {license.id}
          </span>
      ...
      {/* Diagnosis Preview */}
      <div className="bg-[#181d2b] rounded-xl p-3 text-xs text-slate-300 border border-[#2a3247] my-2">
      ...
      {/* Footer link */}
      <div className="flex items-center justify-between pt-3 border-t border-[#2a3247] mt-3 text-xs text-slate-400">
